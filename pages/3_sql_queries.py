...
<div class="sql-header">
    <h1>🧠 SQL Queries</h1>
    <p>Run custom or prebuilt SQL queries on your Cricbuzz MySQL database.</p>
</div>
...
# ✅ Prebuilt queries with full SQL
prebuilt_queries = {
    "1. Players who represent India": "SELECT player_id, full_name, batting_style, bowling_style FROM cricbuzz_db.players WHERE country = 'India';",
    "2. Recent matches": "SELECT match_desc, team1, team2, venue, venue_city, start_date FROM cricbuzz_db.recent_matches ORDER BY start_date DESC LIMIT 10;",
    "3. Top 10 ODI run scorers": "SELECT player_id, player_name, runs FROM cricbuzz_db.top_odi_runs ORDER BY runs DESC LIMIT 10;",
    "4. Venues with capacity > 30000": "SELECT venue_name, city, country, capacity FROM cricbuzz_db.venues WHERE capacity > 30000 ORDER BY capacity DESC;",
    "5. Matches won by each team": "SELECT match_winner, COUNT(*) AS wins FROM cricbuzz_db.combined_matches WHERE match_winner IS NOT NULL AND match_winner <> '' GROUP BY match_winner ORDER BY wins DESC;",
    "6. Count of players by role": "SELECT playing_role, COUNT(*) AS player_count FROM cricbuzz_db.players GROUP BY playing_role ORDER BY player_count DESC;",
    ...
    "9. All-rounders with >1000 runs and >50 wickets": """
        SELECT full_name, total_runs, total_wickets, 'Career' AS format
    ...
    "10. Last 20 completed matches": """
        SELECT 
            ...
            CASE WHEN status LIKE '%won by%' THEN TRIM(SUBSTRING_INDEX(SUBSTRING_INDEX(status, ' won by ', -1), ' ', 1)) ELSE NULL END AS winning_margin,
            CASE WHEN status LIKE '%won by%' AND status LIKE '%wkts%' THEN 'Wickets'
    ...
    "13. Partnerships with 100+ runs": """
        SELECT batter1_name, batter2_name, runs_partnership, innings_no, match_id
        FROM cricbuzz_db.players_partnerships_data
        WHERE runs_partnership >= 100 AND wicket_fallen BETWEEN 1 AND 9
        ORDER BY runs_partnership DESC;
    """,
    "14. Bowling performance at venues": """
        SELECT player_id, player_name, venue, AVG(economy_rate) AS avg_economy, SUM(wickets) AS total_wickets, COUNT(*) AS matches_played
    ...
    "15. Player performance in close matches": """
        SELECT 
            b.player_id, b.player_name, AVG(b.runs) AS avg_runs, COUNT(*) AS close_matches_played,
        ...
        WHERE (
            ...
            OR (cm.win_margin LIKE '%wickets' AND CAST(SUBSTRING_INDEX(cm.win_margin, ' ', 1) AS UNSIGNED) < 5)
        )
        GROUP BY b.player_id, b.player_name
        ORDER BY avg_runs DESC;
    """,
    "16. Batting performance by year": """
        SELECT b.player_id, b.player_name, YEAR(cm.match_date) AS year,
    ...
    "18. Most economical bowlers (ODI + T20)": """
        SELECT player_id, player_name, ROUND(AVG(economy_rate), 2) AS avg_economy, SUM(wickets) AS total_wickets,
               COUNT(*) AS matches_played, ROUND(SUM(overs) / COUNT(*), 2) AS avg_overs_per_match
        FROM cricbuzz_db.bowling_data
        WHERE match_id IN (SELECT match_id FROM cricbuzz_db.combined_matches WHERE LOWER(format) IN ('odi', 't20'))
    ...
    "19. Consistent batsmen": """
        SELECT b.player_id, b.player_name, ROUND(AVG(b.runs), 2) AS avg_runs,
               ROUND(STDDEV(b.runs), 2) AS run_stddev, COUNT(*) AS innings_played
        FROM cricbuzz_db.batting_data b
        JOIN cricbuzz_db.combined_matches cm ON b.match_id = cm.match_id
    ...
    "20. Matches played and batting avg per format": """
        ...
        SELECT b.player_id, cm.format, COUNT(*) AS matches_played,
    ...
    "21. Performance ranking": """
        SELECT b.player_id, cm.format,
               (SUM(b.runs) * 0.01 + (SUM(b.runs) / NULLIF(SUM(CASE WHEN b.dismissal != 'not out' THEN 1 ELSE 0 END), 0)) * 0.5 +
               (SUM(b.runs) * 100 / NULLIF(SUM(b.balls_faced), 0)) * 0.3) AS score
        FROM cricbuzz_db.batting_data b
        JOIN cricbuzz_db.combined_matches cm ON b.match_id = cm.match_id
        GROUP BY b.player_id, cm.format
        UNION ALL
        SELECT bw.player_id, cm.format,
               (SUM(bw.wickets) * 2 + (50 - (SUM(bw.runs_conceded) / NULLIF(SUM(bw.wickets), 0))) * 0.5 +
        ...
        UNION ALL
        SELECT f.player_id, cm.format, (SUM(f.catches) * 1 + SUM(f.stumpings) * 2) AS score
    ...
    "23. Recent form": """
        WITH ranked_stats AS (
            SELECT player_id, match_id, runs, balls_faced,
                   ROW_NUMBER() OVER (PARTITION BY player_id ORDER BY match_id DESC) AS rn
            FROM cricbuzz_db.batting_data
            WHERE runs IS NOT NULL
        ),
        last_10 AS (SELECT * FROM ranked_stats WHERE rn <= 10),
        last_5 AS (SELECT * FROM ranked_stats WHERE rn <= 5),
        stats_10 AS (
            SELECT player_id, ROUND(AVG(runs),2) AS avg_last_10,
        ...
        stats_5 AS (
            SELECT player_id, ROUND(AVG(runs),2) AS avg_last_5
            FROM last_5
            GROUP BY player_id
        )
        SELECT s10.player_id, s5.avg_last_5, s10.avg_last_10, s10.strike_rate_10, s10.fifties_10, s10.consistency_score,
               CASE WHEN s10.avg_last_10 >=50 AND s10.consistency_score <=15 THEN 'Excellent Form'
    ...
    """,
    "24. Best partnerships": """
        SELECT LEAST(batter1_name,batter2_name) AS player1, GREATEST(batter1_name,batter2_name) AS player2,
        ...
        FROM cricbuzz_db.players_partnerships_data
        GROUP BY LEAST(batter1_id,batter2_id), GREATEST(batter1_id,batter2_id),
                 LEAST(batter1_name,batter2_name), GREATEST(batter1_name,batter2_name)
        ORDER BY total_partnerships DESC
        LIMIT 100;
    """,
    "25. Time-series performance evolution": """
        WITH player_quarter_stats AS (
            SELECT player_id, YEAR(date) AS year, QUARTER(date) AS quarter,
                   AVG(runs) AS avg_runs, AVG(strike_rate) AS avg_sr, COUNT(*) AS matches_played
            FROM cricbuzz_db.batters_batting_data
            GROUP BY player_id, YEAR(date), QUARTER(date)
            HAVING COUNT(*)>=1
        ),
        eligible_players AS (
            SELECT player_id FROM player_quarter_stats GROUP BY player_id HAVING COUNT(*)>=6
        ),
        player_trends AS (
            SELECT p1.player_id, CONCAT(p1.year,'-Q',p1.quarter) AS current_qtr,
                   p1.avg_runs AS current_runs, p1.avg_sr AS current_sr,
                   CONCAT(p2.year,'-Q',p2.quarter) AS prev_qtr, p2.avg_runs AS prev_runs, p2.avg_sr AS prev_sr,
                   CASE WHEN p1.avg_runs>p2.avg_runs AND p1.avg_sr>p2.avg_sr THEN 'Improving'
            ...
            JOIN player_quarter_stats p2 
              ON p1.player_id=p2.player_id AND ((p1.year=p2.year AND p1.quarter=p2.quarter+1) OR (p1.year=p2.year+1 AND p1.quarter=1 AND p2.quarter=4))
            WHERE p1.player_id IN (SELECT player_id FROM eligible_players)
        )
        SELECT b.player_name, pt.player_id,
               ...
               CASE WHEN SUM(CASE WHEN pt.performance_trend='Improving' THEN 1 ELSE 0 END)*1.0/COUNT(*)>=0.6 THEN 'Career Ascending'
...
# ✅ Run selected query only (custom input removed)
if st.button("Run Query"):
    if selected_query != "-- Select a query --":
        ...
        st.warning("Please select a query from the dropdown.")
